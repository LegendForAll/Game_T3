...
void CMario::LoadResource()
{
	...
	ani->Add(105, 90);	//ani important
	ani->Add(107, 90);
	ani->Add(108, 90);
	ani->Add(109, 90);
	animations->Add(SM_FLY_ATTACK, ani);

	ani = new CAnimation(100);		// simon after fly
	ani->Add(105);
	animations->Add(SM_AFTER_FLY, ani);
	...
	this->AddAnimation(SM_AFTER_FLY);			//simon after fly
}
...
void CMario::Render(float xViewport, float yViewport)
{
	...
	if (isWP)
	{
		whip->SetPositionLR(x, y, isStand, this->isLeft,this->item);
...
void CMario::SetState(int state)
{
	...
	switch (state)
	{
		...
		case SM_FLY_ATTACK:
		{
			if (animations[SM_FLY_ATTACK]->CheckDoAllFrame())//finish attack
			{
				animations[SM_FLY_ATTACK]->SetDoAllFrame(false);
				this->state = SM_AFTER_FLY;
				ani = SM_AFTER_FLY;
				//this->state = SM_STAND_STATIC;
				//ani = SM_STAND_STATIC;
				isAttack = false;
